...
const RESTAURANT_STORIES = [
  {
    kicker: 'Authentic flavours',
    text: 'Homemade breakfasts, dinners by reservation, local ingredients and a dining room overlooking the hills: an experience designed to complete your stay with the authentic taste of the Tuscan countryside.',
    image: 'https://images.unsplash.com/photo-1414235077428-338989a2e8c0?auto=format&fit=crop&w=1600&q=80'
  },
  {
    kicker: 'Local ingredients',
    text: 'Freshly baked bread, vegetables from the garden, local olive oil and a menu that changes with the seasons shape the stay with a simple, welcoming rhythm.',
    image: 'https://images.unsplash.com/photo-1559339352-11d035aa65de?auto=format&fit=crop&w=1600&q=80'
  },
  {
    kicker: 'Tastings and dinner',
    text: 'Boards to share, local wines and a simple yet thoughtful kitchen turn the evening into a small experience to enjoy slowly, without unnecessary formality.',
...
const API_BASE_URL = (window.__APP_CONFIG__?.API_BASE_URL || '').replace(/\/$/, '');
...
function renderRoomGallery(index = 0) {
  ...
  refs.roomGalleryThumbs.innerHTML = state.roomGalleryImages.map((item, itemIndex) => `
    <button class="room-gallery-thumb${itemIndex === index ? ' is-active' : ''}" type="button" data-gallery-index="${itemIndex}" style="background-image: url('${item}');" aria-label="Image ${itemIndex + 1}"></button>
  `).join('');
...
function buildFeatureStories(property) {
  ...
  return [
    ...
    },
    {
      ...
      eyebrow: 'Experience',
      title: 'A straightforward stay, without confusing steps',
      copy: 'A readable calendar, day-by-day prices and rooms designed for a more elegant and relaxed countryside experience.',
      details: [
...
function updateHeroPeriodDisplay() {
  ...
  refs.heroSelectedPeriod.textContent = 'Choose directly from the calendar below';
}
...
function updateBookingSummary() {
  ...
  if (!checkIn || !checkOut || nightsBetween(checkIn, checkOut) <= 0) {
    refs.bookingTotalValue.textContent = 'Select your stay';
    refs.bookingSummary.innerHTML = `
      <strong>${state.property.name}</strong>
      <p>${state.selectedRoom.name}</p>
      <p>Select check-in and check-out to view your summary.</p>
    `;
    return;
  }
  ...
  refs.bookingTotalValue.textContent = `${formatMoney(totalPrice)} · ${nights} nights`;
  refs.bookingSummary.innerHTML = `
    <strong>${state.property.name}</strong>
    <p>${state.selectedRoom.name}</p>
    <p>${toLongDate(checkIn)} → ${toLongDate(checkOut)}</p>
    <p>${nights} nights · ${guests} guests</p>
    <p><strong>Estimated total from ${formatMoney(totalPrice)}</strong></p>
  `;
}
  ...
  refs.bookingRoomResult.innerHTML = `
    <article class="booking-room-card">
      ...
      <div class="booking-room-copy">
        <p class="eyebrow warm">${mode === 'search' ? 'Available' : 'Selected room'}</p>
        <h3>${room.name}</h3>
        <p>${room.description}</p>
        <div class="booking-room-meta">
          <span class="meta-chip">${room.capacity} guests max</span>
          <span class="meta-chip">${formatMoney(room.base_price)} / night</span>
          ${nights > 0 ? `<span class="meta-chip strong">${nights} nights · ${formatMoney(totalPrice)}</span>` : ''}
        </div>
      </div>
    </article>
  `;
...
function renderBookingCalendar() {
  ...
  state.bookingCalendar.forEach((day) => {
    ...
    const blocked = !day.available || isPastDay(day.day);
...
async function loadHeroCalendar() {
  refs.heroCalendarStatus.textContent = 'Loading availability...';
  refs.heroCalendarStatus.className = 'hero-calendar-status';
  try {
    state.heroCalendar = await fetchPropertyCalendar();
    refs.heroCalendarStatus.textContent = 'Select check-in and check-out from the calendar.';
...
async function loadBookingMonth() {
  ...
  refs.bookingCalendarStatus.textContent = 'Loading calendar...';
  refs.bookingCalendarStatus.className = 'status-copy';
  try {
    state.bookingCalendar = await fetchRoomCalendar(state.selectedRoom.id);
    refs.bookingCalendarStatus.textContent = 'Select check-in and check-out directly from the calendar.';
...
function renderRooms(rooms) {
  ...
  if (!rooms.length) {
    refs.roomsGrid.innerHTML = '<p class="status-copy">No rooms available.</p>';
    return;
  }
  ...
  for (const room of rooms) {
    ...
    card.querySelector('.room-price').textContent = `${formatMoney(room.base_price)} / night`;
    card.querySelector('.room-meta').innerHTML = `
      <span class="meta-chip">${room.capacity} guests max</span>
      <span class="meta-chip">Dynamic rate shown in the calendar</span>
    `;
...
async function openBookingDialog(room) {
  ...
  refs.bookingActivate.textContent = 'Book this room';
...
async function handleAvailabilitySearch() {
  if (!state.filters.checkIn || !state.filters.checkOut || !state.filters.guests) {
    setStatus('Enter check-in, check-out and guests to continue.', 'error');
    return false;
  }
  const rooms = await fetchAvailableRooms();
  if (!rooms.length) {
    setStatus('No room available for these dates.', 'error');
    return false;
  }
...
async function submitBooking(event) {
  ...
  const guests = Number(state.bookingDraft?.guests || state.filters.guests || 1);
  if (!checkIn || !checkOut || nightsBetween(checkIn, checkOut) <= 0) {
    refs.bookingFormStatus.textContent = 'Select a valid stay before confirming.';
    refs.bookingFormStatus.className = 'status-copy is-error';
    return;
  }
  const nights = nightsBetween(checkIn, checkOut);
  refs.bookingFormStatus.textContent = 'Sending booking...';
  refs.bookingFormStatus.className = 'status-copy';
  try {
    ...
    refs.bookingFormStatus.textContent = `Bookzione confermata: ${booking.room_name}`;
...
function submitContactForm(event) {
  ...
  if (!name || !email || !message) {
    refs.contactFormStatus.textContent = 'Fill in name, email and message to send your request.';
    refs.contactFormStatus.className = 'status-copy is-error';
    return;
  }

  refs.contactFormStatus.textContent = 'Request sent. We will get back to you shortly.';
...
async function handleSearch(event) {
  ...
  try {
    ...
    if (!state.filters.checkIn || !state.filters.checkOut || nightsBetween(state.filters.checkIn, state.filters.checkOut) <= 0) {
      setStatus('Select your stay dal calendario prima di continuare.', 'error');
      return;
    }
    ...
    if (opened) {
      setStatus('Available room found. You can complete the booking in the popup.', 'success');
    }
  } catch (error) {
    setStatus(error.message || 'I could not verify availability.', 'error');
  }
}
...
async function bootstrap() {
  ...
  updateHeroPeriodDisplay();
  setStatus('Select your dates and choose a room.', 'success');
}
